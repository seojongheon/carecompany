...
export function CaseLightbox({ media, index, onClose }: { media: CaseMedia[]; index: number; onClose(): void }) {
  return <Lightbox open={index >= 0} close={onClose} index={Math.max(0, index)} plugins={[Captions, Zoom]} slides={media.map((item) => ({ src: MOCK_ASSETS[item.mockAssetKey], alt: item.altText, title: item.caption, description: item.altText, width: item.width, height: item.height, stage: item.stage }))} render={{ slideHeader: ({ slide }) => <MediaStageBadge stage={(slide as StageSlide).stage} className="right-4 top-20" testId="case-lightbox-stage-badge" /> }} carousel={{ finite: true }} controller={{ closeOnBackdropClick: true }} />;
}
